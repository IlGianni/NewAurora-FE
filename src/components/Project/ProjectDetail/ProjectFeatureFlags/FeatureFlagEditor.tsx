...
export default function FeatureFlagEditor() {
  const { id, groupId, flagKey } = useParams<{
    id: string;
    groupId: string;
    flagKey: string;
  }>();
  ...
  const [flagIdState, setFlagIdState] = useState<string | null>(null);

  // Targeting states - estende Feature_Flag_Target con id temporaneo
  const [targetingRules, setTargetingRules] = useState<
    Array<
      Omit<
        ...
        | "feature_flag"
      > & {
        id: string | number | null; // string/number se esistente, stringa temporanea (temp-*) se nuovo
      }
    >
  >([]);

  // Rules states - estende Feature_Flag_Rule con id temporaneo
  ...
  // Fetch della feature flag basato sull'URL
  useEffect(() => {
    fetchFeatureFlag();
  }, [id, flagKey]);
  ...
  async function fetchFeatureFlag() {
    ...
    // Se abbiamo flagKeyToUse, fetcha la feature flag esistente
    if (
      id &&
      flagKey !== null &&
      !window.location.pathname.includes("/feature-flags/new-flag/")
    ) {
      try {
        const response = await axios.get("/project/GET/get-feature-flag", {
          params: {
            projectUniqueId: id,
        ...
        if (response.status === 200 && response.data) {
          console.log("Feature flag data:", response.data);
          const flagData = (response.data.feature_flag ||
            response.data) as Feature_Flag;

          // Popola gli stati base
          setFlagId(
            flagData.feature_flag_id ? String(flagData.feature_flag_id) : null
          );
          setEditFlagName(flagData.name || "");
          setEditFlagKey(flagData.key || "");
          setEditFlagDescription(flagData.description || "");
          setLocalEnabled(
            flagData.enabled !== undefined ? flagData.enabled : false
          );
          setDefaultRule(
            flagData.default_value !== undefined ? flagData.default_value : true
          );
          setFlagIdState(
            flagData.feature_flag_id ? String(flagData.feature_flag_id) : null
          );

          // Mappa i targets - solo quelli con id dal server (esistenti)
          const targets = Array.isArray(flagData.targets)
            ? flagData.targets
                .filter((target: Feature_Flag_Target) => target.target_id) // Solo quelli con id
                .map((target: Feature_Flag_Target) => ({
                  id: target.target_id, // Usa solo l'id del server
                  name: target.name || "",
                  type: target.type || "user",
                  operator: target.operator || "equals",
                  value: target.value || "",
                  enabled:
                    target.enabled !== undefined ? target.enabled : false,
                }))
            : [];
          setTargetingRules(targets);

          // Mappa le regole - solo quelle con id dal server (esistenti)
          const rulesData = Array.isArray(flagData.feature_flag_rules)
            ? flagData.feature_flag_rules
                .filter((rule: Feature_Flag_Rule) => rule.feature_flag_rule_id) // Solo quelle con id
                .map((rule: Feature_Flag_Rule) => ({
                  id: rule.feature_flag_rule_id, // Usa solo l'id del server
                  field: rule.field || "",
        ...
        console.error("Errore durante il fetch della feature flag:", error);
        setEditFlagName("");
        setEditFlagKey("");
        setEditFlagDescription("");
  ...
  const addTargetingRule = () => {
    const newTargetingRule = {
      id: `temp-${Date.now()}-${Math.random()}`, // ID temporaneo univoco
      name: "",
      type: "user",
      operator: "equals",
      value: "",
  ...
  const removeTargetingRule = async (id: string | number | null) => {
    // Se è un ID temporaneo, rimuovi solo dallo stato locale
    if (id && typeof id === "string" && id.startsWith("temp-")) {
      setTargetingRules(targetingRules.filter((rule) => rule.id !== id));
      return;
    }
    // Se è un ID reale, elimina dal server
    if (id) {
      await axios.delete(`/project/DELETE/delete-feature-flag-target`, {
        data: {
          feature_flag_target_id: id,
        },
      });
    }
    setTargetingRules(targetingRules.filter((rule) => rule.id !== id));
  ...
  const removeRule = async (id: string | number | null) => {
    // Se è un ID temporaneo, rimuovi solo dallo stato locale
    if (id && typeof id === "string" && id.startsWith("temp-")) {
    ...
    // Se è un ID reale, elimina dal server
    if (id) {
      await axios.delete(`/project/DELETE/delete-feature-flag-rule`, {
        data: {
          feature_flag_rule_id: id,
        },
      });
    }
    setRules(rules.filter((rule) => rule.id !== id));
  ...
  // Genera una key valida dal nome della feature flag
  ...
  // Genera automaticamente la key quando il nome cambia (solo se la key è vuota e non è una flag esistente)
  useEffect(() => {
    if (editFlagName && !editFlagKey && !flagIdState) {
      const generatedKey = generateKeyFromName(editFlagName);
      setEditFlagKey(generatedKey);
    }
  ...
  // Verifica se ci sono regole incomplete
  const hasIncompleteRules = () => {
  ...
  // Verifica se ci sono target incomplete
  const hasIncompleteTargets = () => {
    if (targetingRules.length === 0) return false;
    return targetingRules.some(
      (target) =>
        !target.name?.trim() ||
        !target.type?.trim() ||
        !target.operator?.trim() ||
        !target.value?.trim()
    );
  };

  // Verifica se è possibile salvare
  const canSave = () => {
    // Verifica che il nome non sia vuoto
    if (!editFlagName.trim()) return false;
    // Verifica che non ci siano regole incomplete
    if (hasIncompleteRules()) return false;
    // Verifica che non ci siano target incomplete
  ...
  const handleSave = async () => {
    ...
    try {
      const feature_flag_data = {
        ...
        feature_flag_group_id: groupId === "null" ? null : groupId,
        targeting: targetingRules.map((rule) => {
          ...
          // Includi l'id solo se esiste ed è un ID reale (non temporaneo)
          if (
            rule.id &&
            !(typeof rule.id === "string" && rule.id.startsWith("temp-"))
          ) {
            targetData.target_id = rule.id;
          }
          return targetData;
        }),
        ...
        default_value: defaultRule,
      };

      if (flagIdState) {
        // Aggiorna feature flag esistente
        const response = await axios.put(
          "/project/UPDATE/update-feature-flag",
          {
            id: flagIdState,
        ...
        // Crea nuova feature flag
        const response = await axios.post("/project/POST/create-feature-flag", {
          feature_flag_data: feature_flag_data,
        });
      ...
      console.error("Errore durante il salvataggio della feature flag:", error);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold text-default-900">
                {editFlagName || "Nuova Feature Flag"}
              </h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <CardBody className="gap-4 p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Input
                ...
                placeholder="es. nuova-feature-flag"
                endContent={
                  <Button
                    isIconOnly
                    variant="light"
                    ...
                    onPress={() => {
                      if (editFlagName) {
                        setEditFlagKey(generateKeyFromName(editFlagName));
                      }
                    }}
                    className="min-w-6 w-6 h-6"
                    title="Genera key dal nome"
                  >
        ...
        >
          ...
          >
            <div className="py-6 space-y-5">
                  ...
                  <CardBody className="gap-3 p-4">
                    ...
                    <div className="space-y-2">
                      {targetingRules.map((target, index) => (
                        <div
                          key={target.id || `new-target-${index}`}
                        ...
                        >
                          ...
                          <Switch
                            ...
                            onValueChange={(value) =>
                              updateTargetingRule(target.id, "enabled", value)
                            }
                          />
                        </div>
                      ))}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-4">
                    {targetingRules.map((targetingRule, index) => {
                      const isIncomplete =
                        !targetingRule.name?.trim() ||
                        !targetingRule.type?.trim() ||
                        !targetingRule.operator?.trim() ||
                        !targetingRule.value?.trim();
                      return (
                        <Card
                          key={targetingRule.id || `new-target-${index}`}
                          shadow="none"
                          className={`border ${
                            isIncomplete
              ...
              {hasIncompleteTargets() && targetingRules.length > 0 && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-warning-50 border border-warning-200">
                  ...
                  <p className="text-xs text-warning-700">
                    Completa tutti i target prima di salvare. Ogni target deve
                    avere nome, tipo, operatore e valore compilati.
                  </p>
                </div>
              )}
              ...
              {targetingRules.length === 0 && (
                <div className="text-center py-8 text-sm text-default-500">
                  ...
                  <p className="text-xs mt-1">
                    Aggiungi un target per iniziare
                  </p>
                </div>
              )}
            </div>
          ...
          >
            <div className="py-6 space-y-5">
              ...
              {hasIncompleteRules() && rules.length > 0 && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-warning-50 border border-warning-200">
                  <Icon
                    icon="lucide:alert-circle"
                    width={16}
                  ...
                  />
                  <p className="text-xs text-warning-700">
                    Completa tutte le regole prima di salvare. Ogni regola deve
                    avere campo, operatore e valore compilati.
                  </p>
                </div>
              )}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Icon
                        ...
                        width={16}
                        className={
                          localEnabled ? "text-success" : "text-danger"
                        }
                      />
                      <span className="text-xs font-medium text-default-900">
                        Feature Flag
                    ...
                    <div className="flex justify-center">
                      ...
                      >
                        <Icon
                          ...
                          width={12}
                          className={
                            defaultRule ? "text-success" : "text-danger"
                          }
                        />
                ...
                {targetingRules.length > 0 ? (
                  <div className="p-4 rounded-xl border border-default-200 bg-white space-y-3">
                    {targetingRules.map((target, index) => {
                      ...
                      const actualValue = target.enabled
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="w-full h-2 bg-default-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full ${
                                target.enabled ? "bg-primary" : "bg-default-300"
                ...
                <div className="p-4 rounded-xl border border-primary-200 bg-white space-y-3">
                  ...
                  {targetingRules.length > 0 ? (
                    <div className="space-y-2">
                      {(() => {
                        // Conta gli operatori
                        const operatorCounts = targetingRules.reduce(
                          (acc, t) => {
