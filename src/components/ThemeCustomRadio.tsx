...
export const ThemeCustomRadio = (props: ThemeCustomRadioProps) => {
  ...
  return (
    <Component
      {...getBaseProps()}
      className={cn(
        "group hover:bg-content2 cursor-pointer",
        "rounded-large border-default-200 max-w-[300px] border-2 shadow-sm",
        "relative h-[132px] flex-1 overflow-hidden transition-all duration-200",
        "hover:border-primary/50 hover:shadow-md",
        "group-data-[selected=true]:border-primary group-data-[selected=true]:bg-primary/5",
        "group-data-[selected=true]:shadow-md"
      )}
    >
      <VisuallyHidden>
        <input {...getInputProps()} />
      ...
      {/* Radio button in alto a destra */}
      <span
        {...getWrapperProps()}
        className={cn(
          wrapperProps["className"],
          "absolute top-3 right-3 z-20",
          "border-default-300 dark:border-default-600 border-2",
      ...
      >
        <span
          {...getControlProps()}
          className={cn(
            "bg-primary text-primary-foreground transition-all duration-200 z-10 h-2.5 w-2.5 origin-center scale-0 rounded-full opacity-0 group-data-[selected=true]:scale-100 group-data-[selected=true]:opacity-100"
          )}
        />
      </span>
      ...
      <div
        {...getLabelWrapperProps()}
        className="flex flex-col justify-start absolute top-3 left-4 z-20"
      >
        {children && (
          <span
            {...getLabelProps()}
            className="text-sm font-semibold text-foreground group-data-[selected=true]:text-primary"
          >
            {children}
          </span>
        )}
        {description && (
          <span className="text-xs text-default-500 mt-1">{description}</span>
        )}
      </div>
      <div
